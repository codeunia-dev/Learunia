  ...
  return (
    <div className="relative max-w-2xl mx-auto" ref={searchRef}>
      <form onSubmit={handleSubmit}>
        <div className="relative">
          <input
            ...
            className="w-full px-6 py-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-white placeholder-[#D1D1D1] focus:outline-none focus:ring-2 focus:ring-[#007AFF] focus:border-transparent transition-all duration-200"
            autoComplete="off"
          />
          <button
            type="submit"
            className="absolute right-4 top-1/2 transform -translate-y-1/2 p-1 hover:text-[#007AFF] transition-colors"
            aria-label="Search"
          >
        ...
        <div className="absolute top-full mt-2 w-full bg-[#1A1A2E] border border-white/20 rounded-xl shadow-2xl backdrop-blur-sm z-50 overflow-hidden">
          {filteredResults.map((result) => (
            <button
              ...
              className="w-full px-6 py-3 text-left hover:bg-white/10 transition-colors border-b border-white/10 last:border-b-0 group"
            >
              <div className="flex items-center justify-between">
                <span className="text-white group-hover:text-[#007AFF] transition-colors">
                  {result.name}
                </span>
      ...
      {isOpen && query.length > 0 && filteredResults.length === 0 && (
        <div className="absolute top-full mt-2 w-full bg-[#1A1A2E] border border-white/20 rounded-xl shadow-2xl backdrop-blur-sm z-50 px-6 py-4">
          <p className="text-[#D1D1D1] text-center">No cheatsheets found for &quot;{query}&quot;</p>
        </div>
      )}
    </div>
  );
